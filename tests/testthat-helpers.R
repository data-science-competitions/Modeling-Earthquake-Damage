...
expect_an_empty_data.frame <- function(x){expect_class(x, "data.frame"); expect(nrow(x) == 0, error_message("data.frame is not-empty."))}
expect_a_non_empty_data.frame <- function(x){expect_class(x, "data.frame"); expect(nrow(x) > 0, error_message("data.frame is empty."))}
