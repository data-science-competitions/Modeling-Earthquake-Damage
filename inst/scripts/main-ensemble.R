...
output_dir <- file.path(getOption("path_archive"), "stacked-models")
...
# Run Learners ------------------------------------------------------------
iterators <- expand.grid(model_name = model_names, split = 1:get_rsample_num_of_splits(rset_obj), stringsAsFactors = FALSE)
responses <- list()
for(i in seq_len(nrow(iterators))){
    model_name <- iterators[i, "model_name"]
    split <- iterators[i, "split"]
    message("| model_name: ", model_name, " | split: ", split, " |")

    train_set <-
        get_rsample_training_set(rset_obj, split) %>%
        dplyr::select(-dplyr::starts_with(".")) %>%
        dplyr::select(role_pk, role_input, role_target, role_none)

    test_set <-
        get_rsample_test_set(rset_obj, split) %>%
        dplyr::select(-dplyr::starts_with(".")) %>%
        dplyr::select(role_pk, role_input, role_target, role_none)
    ...
    model_response <- pm$response %>% dplyr::select(role_pk, fit) %>% dplyr::rename(!!model_name := fit)
    responses <- append(responses, list(model_response))
}
level_1 <- responses %>% dplyr::bind_rows() %>% group_by_at(role_pk) %>% dplyr::summarise_all(sum, na.rm = TRUE)
